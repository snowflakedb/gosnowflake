...
Clients can use the database/sql package directly. For example:
	...
	func main() {
		db, err := sql.Open(DriverRegistrationName(), "user:password@my_organization-my_account/mydb")
		if err != nil {
			log.Fatal(err)
		}
		defer db.Close()
...
Use the Open() function to create a database handle with connection parameters:
	...
	db, err := sql.Open(DriverRegistrationName(), "<connection string>")
...
schema is "testschema", and warehouse is "mywh":
	...
	db, err := sql.Open(DriverRegistrationName(), "jsmith:mypassword@my_organization-my_account/mydb/testschema?warehouse=mywh")
