...
func TestBindingsWithSameValue(t *testing.T) {
	...
	runDBTest(t, func(dbt *DBTest) {
		...
		intArr = make([]int, numRows*80)
		strArr = make([]string, numRows*80)
		timeArr = make([]time.Time, numRows*80)
		boolArr = make([]bool, numRows*80)
		doubleArr = make([]float64, numRows*80)

		for k := 0; k < 80; k++ {
			for i := 0; i < numRows; i++ {
				intArr[(k*numRows)+i] = (k * numRows) + i
				doubleArr[(k*numRows)+i] = doubleAnyArr[i]
				strArr[i] = "test" + strconv.Itoa(i)
				boolArr[i] = boolAnyArr[i]
				timeArr[i] = timeAnyArr[i].(time.Time)
			}
		}
