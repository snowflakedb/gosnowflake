...
func setup() (string, error) {
	...
	var db *sql.DB
	var err error
	if db, err = sql.Open(DriverRegistrationName(), dsn); err != nil {
...
func invalidUserPassErrorTests(invalidDNS string, t *testing.T) {
	...
	db, err := sql.Open(DriverRegistrationName(), invalidDNS)
	if err != nil {
...
func TestValidateDatabaseParameter(t *testing.T) {
	...
	for idx, tc := range testcases {
		t.Run(dsn, func(t *testing.T) {
			...
			db, err := sql.Open(DriverRegistrationName(), newDSN)
			// actual connection won't happen until run a query
			if err != nil {
				t.Fatalf("error creating a connection object: %s", err.Error())
			}
...
func TestSpecifyWarehouseDatabase(t *testing.T) {
	...
	db, err := sql.Open(DriverRegistrationName(), dsn+"?"+parameters.Encode())
	if err != nil {
		t.Fatalf("error creating a connection object: %s", err.Error())
	}
	defer db.Close()
...
func TestPingInvalidHost(t *testing.T) {
	...
	db, err := sql.Open(DriverRegistrationName(), testURL)
	if err != nil {
