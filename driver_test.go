...
func TestArrayBind(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		dbt.mustExec("create or replace table test(c1 int, c2 float, c3 boolean, c4 string)")
		intArray := []int{1, 2, 3}
		fltArray := []float64{0.1, 2.34, 5.678}
		boolArray := []bool{true, false, true}
		strArray := []string{"test1", "test2", "test3"}
		dbt.mustExec(`insert into test values (?, ?, ?, ?)`, intArray, fltArray, boolArray, strArray)
