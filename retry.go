...
func (r *retryHTTP) execute() (res *http.Response, err error) {
	...
	logger.Infof("retryHTTP.totalTimeout: %v", totalTimeout)
	retryCounter := 0
	...
	for {
		...
		if totalTimeout > 0 {
			logger.Infof("to timeout: %v", totalTimeout)
		...
		logger.Infof("sleeping %v. to timeout: %v. retrying", sleepTime, totalTimeout)
