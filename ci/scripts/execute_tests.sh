...
if [[ -n "$JENKINS_HOME" ]]; then
  ...
  go test $GO_TEST_PARAMS -timeout 50m -race -v . | go-junit-report -iocopy -out $WORKSPACE/junit-go.xml
else
  go test $GO_TEST_PARAMS -timeout 50m -race -coverprofile=coverage.txt -covermode=atomic -v .
fi
