...
func TestSoteriaOauthSnowflakeAuthorizationCodeSuccessful(t *testing.T) {
	...
	loginCredentials := getAuthLoginTestCredentials(t)
	var wg sync.WaitGroup
	wg.Add(2)
	go func() {
		defer wg.Done()
		provideExternalBrowserCredentials(t, externalBrowserType.OauthSnowflakeSuccess, loginCredentials.User, loginCredentials.Password)
	}()
	go func() {
		defer wg.Done()
...
func TestSoteriaOauthSnowflakeAuthorizationCodeOktaTimeout(t *testing.T) {
	...
	err := verifyConnectionToSnowflakeAuthTests(t, cfg)
	assertEqualE(t, err.Error(), "authentication via browser timed out")
}
