...
func checkOCSPCacheServer(
	...
	res, err := newRetryHTTP(ctx, client, req, ocspServerHost, headers, totalTimeout, defaultRetryLoginTimeout, defaultMaxRetryCount, defaultTimeProvider, nil).execute()
	if err != nil {
...
func retryOCSP(
	...
	res, err := newRetryHTTP(
		...
		totalTimeout*time.Duration(multiplier), defaultRetryLoginTimeout, defaultMaxRetryCount, defaultTimeProvider, nil).doPost().setBody(reqBody).execute()
	if err != nil {
...
func fallbackRetryOCSPToGETRequest(
	...
	res, err := newRetryHTTP(ctx, client, req, ocspHost, headers,
		totalTimeout*time.Duration(multiplier), defaultRetryLoginTimeout, defaultMaxRetryCount, defaultTimeProvider, nil).execute()
	if err != nil {
		return ocspRes, ocspResBytes, &ocspStatus{
			code: ocspFailedSubmit,
			err:  err,
