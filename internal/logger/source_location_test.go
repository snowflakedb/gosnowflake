...
// IMPORTANT: The skip depth values in defaultLogger and slogEntry assume the standard wrapper chain:
// For logger methods: levelFilteringLogger -> secretMaskingLogger -> defaultLogger (skip=3)
...
// - internal/logger/slog_logger.go: defaultLogger methods (currently skip=3)
...
// with the standard wrapper chain: levelFilteringLogger -> secretMaskingLogger -> defaultLogger
func TestSourceLocationWithLevelFiltering(t *testing.T) {
	innerLogger := NewDefaultLogger()
	var buf bytes.Buffer
	innerLogger.SetOutput(&buf)
	_ = innerLogger.SetLogLevel("debug")

	// Build the standard wrapper chain
	maskedInterface := NewSecretMaskingLogger(innerLogger)
	masked, _ := maskedInterface.(SFLogger)
...
func TestSkipDepthWarning(t *testing.T) {
	...
	// Log from this test
	filtered.Debug("skip depth test") // Line 106 - This line should appear in source location

	output := buf.String()

	if !strings.Contains(output, "source_location_test.go:106") {
		t.Errorf(`
Skip depth appears incorrect!

Expected source location: source_location_test.go:106
Got: %s
...
Current wrapper chain for logger methods:
  Driver code -> levelFilteringLogger -> secretMaskingLogger -> defaultLogger
