...
func TestParsePrivateKeyFromFileIncorrectData(t *testing.T) {
	pemFile := createTmpFile([]byte("gibberish"))
...
func createTmpFile(content []byte) string {
	tempFile, _ := os.CreateTemp("", "exampleKey.pam")
...
func TestGetConfigFromEnv(t *testing.T) {
	...
	for key, ctv := range envMap {
		...
		properties = append(properties, ConfigParam{ctv.configParam, key, true})
	}
	defer func() {
	...
	errors := isConfigCorrect(*cfg, envMap)
	if errors != "" {
		t.Errorf(errors)
	}
}

func isConfigCorrect(cfg Config, envMap map[string]configParamToValue) string {
	...
	for k, ctv := range envMap {
		if ctv.configParam == "Port" {
			...
			if portStr != ctv.value {
				errors = errors + fmt.Sprintf("Field %v expected value: %v, received value %v\n", k, ctv.value, cfgValues[ctv.configParam])
			}
