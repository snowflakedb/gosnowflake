...
func getChunk(
	...
	*http.Response, error,
	...
	return newRetryHTTP(ctx, sc.rest.Client, http.NewRequest, u, headers, timeout, sc.rest.RetryLoginTimeout, sc.rest.MaxRetryCount, sc.currentTimeProvider, sc.cfg).execute()
}
...
func (f *httpStreamChunkFetcher) fetch(URL string, rows chan<- []*string) error {
	...
	res, err := newRetryHTTP(context.Background(), f.client, http.NewRequest, fullURL, f.headers, 0, 0, 0, defaultTimeProvider, nil).execute()
	if err != nil {
		return err
	}
