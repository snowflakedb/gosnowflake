...
func TestWithOauthAuthorizationCodeFlowManual(t *testing.T) {
	...
	for _, provider := range []string{"OKTA", "SNOWFLAKE"} {
		t.Run(provider, func(t *testing.T) {
			...
			err = credentialsStorage.deleteCredential(newOAuthAccessTokenSpec(tokenRequestURL, cfg.User))
			assertNilF(t, err)
			err = credentialsStorage.deleteCredential(newOAuthRefreshTokenSpec(tokenRequestURL, cfg.User))
			assertNilF(t, err)
			connector := NewConnector(&SnowflakeDriver{}, *cfg)
			db := sql.OpenDB(connector)
			defer db.Close()
			...
			err = credentialsStorage.setCredential(newOAuthAccessTokenSpec(cfg.OauthTokenRequestURL, cfg.User), "expired-token")
			assertNilF(t, err)
